...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchUserData = async (userId: string) => {
    setLoading(true);
    try {
      // FIX: Changed from 'users' to 'profiles' to match DB schema
  ...
  const refreshFeed = async () => {
    ...
    // FIX: Changed 'user:users(*)' to 'user:profiles(*)' to match DB schema
  ...
  const toggleSaveRestaurant = async (restaurantId: string): Promise<boolean> => {
    ...
    if (supabaseUrl === 'https://placeholder.supabase.co') {
      const wantList = lists.find(l => l.is_default);
      if (wantList) {
        const exists = wantList.items?.includes(restaurantId);
        if (exists) {
          setLists(prev => prev.map(l => l.id === wantList.id 
    ...
    let wantToGoList = lists.find(l => l.is_default);
